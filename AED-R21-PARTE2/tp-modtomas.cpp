...
// FIN ESTRUCTURA RANKING DE GOLEADORES
...
struct nodoArbol{
  ...
  // CONSTRUCTOR DE MI FUNCION PARA INSERTAR JUGADOR EN LISTA
void insertarJugadorEnRank(lstRankFechas *&, nodoLista *);
...
/* TODO:INSERTAR ORDENANDO, REFACTOREAR PARA LAS NUEVAS LISTAS

void insertarOrdenado(){
  //Creo un nodo del tipo voto (con el voto dentro)
  dvoto *nuevo = new dvoto;
  nuevo -> dato = v;
  nuevo -> siguiente = NULL;

  //Si la lista de nodos estaba vacia asignarla al nuevo nodo
  if(dv == NULL){
    dv = nuevo;
    return;
  }

  //Comparo con el primer nodo, si la edad es menor al primer nodo lo inserto al principio de la lista
  if(dv -> dato.edad > nuevo -> dato.edad){
    nuevo -> siguiente = dv;
    dv = nuevo;
    return;
  }

  dvoto *vaux = dv; //Auxiliar para recorrer
  //Si la lista no esta vacia insertar el nuevo nodo ordenado por edad
  while(vaux -> siguiente != NULL && vaux -> siguiente -> dato.edad < nuevo -> dato.edad){
    vaux = vaux -> siguiente;
  }
  if(vaux -> siguiente == NULL){
    vaux -> siguiente = nuevo;
  }else{
    nuevo -> siguiente = vaux -> siguiente;
    vaux -> siguiente = nuevo;
  }
}
*/
...
void insertarJugadorEnRank(lstRankFechas *&cl, nodoLista *nl){
  ...
  lrfAux->siguiente = new lstRankFechas;
  lrfAux = lrfAux->siguiente;
  lrfAux = new lstRankFechas;
  lrfAux->fecha.fecha = nl -> fecha;
  lrfAux->fecha.lstRankJugadores = new lstRankJugador;
  strcpy(lrfAux->fecha.lstRankJugadores->jugador.nombreJugador, nl->nombreJugador);
  lrfAux->fecha.lstRankJugadores->jugador.goles= nl->goles;
  return;
}


void recorrerMatrizGoles (matriz mPartidos,equipos e[]){
  cout << "-------- RESULTADO RANKING --------" << endl;
  ...
  for(x=0; x<32; x++){ // Cada iteracion corresponde a un equipo
    ...
    for(y=0; y<7; y++){ // Recorro los 7 posibles partidos del equipo en el mundial
      ...
      cout << "\tPartido " << y+1 << " (" << mPartidos[x][y] -> fecha << ")" << endl; // Encontre un partido con goles para un equipo
      while(auxNL != NULL){ // Recorro mi lista de goles para ese partido
        ...
        //insertarJugadorEnRank(cabezaRanking, auxNL);
        auxNL = auxNL -> siguiente;
      }
    }
